...
let numberOfMovies = movies.filter(
  (numberOfMovies) => numberOfMovies.year > 1979 && numberOfMovies.year < 1990
).length;
...
const searchKeyWords = movies.filter(
  (movie) =>
    movie.title.match(/alien/gi) ||
    movie.title.match(/surfer/gi) ||
    movie.title.match(/benjamin/gi)
);
console.log(
  "There are " +
    searchKeyWords.length +
    " movies having keywords Surfer, Alien or Benjamin in title"
);
