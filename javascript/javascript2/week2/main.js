...
const userInput = document.getElementsByClassName("search");
const userOption = document.getElementByClas("selectOption");
...
userInput.addEventListener("keyup", () => {
    const searchAProduct = document.getElementsByClassName("search").value.toLowerCase().trim();
    return searchAProduct ? renderProducts(products.filter(a => a.name.toLowerCase().includes(searchAProduct))) :
        renderProducts(products)
})
userOption.addEventListener("change", () => {
    const searchedOption = document.getElementById("selectOption").value;
    return searchedOption == "lowToHigh" ? renderProducts(products.sort((a, b) => a.price - b.price)) :
        (searchedOption == "highToLow" ? renderProducts(products.sort((a, b) => b.price - a.price)) :
...
userPriceRange.addEventListener("change", () => {
    ...
    return searchedPrice ? renderProducts(products.filter(a => a.price < searchedPrice)) :
        renderProducts(products);
})